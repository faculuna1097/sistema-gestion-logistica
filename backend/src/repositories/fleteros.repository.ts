...
export async function getAll(): Promise<Fletero[]> {
  ...
  const result = await pool.query<Fletero>(
    'SELECT id, nombre, periodo_vencimiento FROM fleteros ORDER BY nombre ASC'
  );
...
export async function getById(id: number): Promise<Fletero | null> {
  ...
  const result = await pool.query<Fletero>(
    'SELECT id, nombre, periodo_vencimiento FROM fleteros WHERE id = $1',
    [id]
  );
...
export async function remove(id: number): Promise<Fletero | null> {
  ...
  const result = await pool.query<Fletero>(
    `DELETE FROM fleteros WHERE id = $1
     RETURNING id, nombre, periodo_vencimiento`,
    [id]
  );
